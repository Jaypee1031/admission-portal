...
if (empty($results)) {
    die('No test results found to print.');
}

// Use the same CATResultPDF layout as single print_test_result.php
$pdfGenerator = new CATResultPDF();
$pdf = null;

foreach ($results as $result) {
    // Each call adds a new page with the standard CAT result layout
    $pdf = $pdfGenerator->generateCATResult($result);
}

if ($pdf) {
    $filename = 'All_Test_Results_' . date('Y-m-d') . '.pdf';
    $pdf->Output($filename, 'D');
    exit;
} else {
    die('No valid test results found to print.');
}
?>
