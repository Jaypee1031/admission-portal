...
require_once '../includes/pdf_favicon_helper.php';
...
if (empty($students)) {
    die('No Personal Data forms found to print.');
}
...
foreach ($students as $student) {
    // Convert individual columns to the expected format expected by F2PersonalDataPDF
    $formData = [
        'personal_info' => [
...
if ($pdf) {
    $filename = 'All_F2_Personal_Data_Forms_' . date('Y-m-d') . '.pdf';
    $pdf->Output($filename, 'D');
    exit;
} else {
    die('No valid Personal Data forms found to print.');
}
?>
