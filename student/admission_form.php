...
if (!isStudent()) {
    ...
    if (isset($_GET['iframe']) || (isset($_SERVER['HTTP_REFERER']) && strpos($_SERVER['HTTP_REFERER'], 'dashboard.php') !== false)) {
        // If in iframe, show error message instead of redirecting
...
// Clear any existing alerts to prevent confusion
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $uploader    = new AdmissionFormFileUploader($admissionForm, $fileHelper);
...
<body class="bg-light">
        ...
        <div class="row mb-3">
            <div class="col-12">
                <div class="alert alert-info">
                    ...
                    Form Data: <?php echo $formData ? 'Available' : 'Not Available'; ?> | 
                    Courses: <?php echo count($courses); ?> | 
                    Strands: <?php echo count($strands); ?> |
                    Alert: <?php echo $alert ? 'Present' : 'None'; ?>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        // Auto-calculate age from birth date (only if age field is empty)
        document.getElementById('birth_date').addEventListener('change', function() {
            const ageField = document.getElementById('age');
            
            // Only auto-calculate if age field is empty or user hasn't manually entered a value
            if (!ageField.value || ageField.value === '') {
                ...
                setTimeout(() => {
                    ...
                    ageField.style.borderColor = '#ced4da';
                }, 1000);
            }
        });
        ...
        // Calculate age on page load if birth date is already filled
        document.addEventListener('DOMContentLoaded', function() {
            const birthDateField = document.getElementById('birth_date');
            const ageField = document.getElementById('age');
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Handle year_last_attended field based on student type
            const yearLastAttendedField = document.getElementById('year_last_attended');
            const studentType = '<?php echo $user['student_type']; ?>';
            
            if (yearLastAttendedField) {
                if (studentType === 'Transferee') {
                    // For transferee students, field is required
                    yearLastAttendedField.required = true;
                    
                    yearLastAttendedField.addEventListener('input', function() {
                        const value = this.value.trim();
                        if (value === '' || value === '0') {
                            this.setCustomValidity('Year Last Attended is required for transferee students');
                        } else {
                            this.setCustomValidity('');
                        }
                    });
                    
                    yearLastAttendedField.addEventListener('blur', function() {
                        const value = this.value.trim();
                        if (value === '' || value === '0') {
                            this.setCustomValidity('Year Last Attended is required for transferee students');
                            return;
                    ...
                    // For freshmen, field is hidden and not required
                    yearLastAttendedField.required = false;
                }
            }
